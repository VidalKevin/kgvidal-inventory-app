...
function normalizeDays(value: unknown, frequency: SyncFrequency) {
  if (frequency === "daily") return DAY_CODES;
  const values = Array.isArray(value) ? value : [];
  const days = values.map((item) => String(item)).filter((item) => DAY_CODES.includes(item));
  return days.length ? Array.from(new Set(days)) : defaultShipheroOnholdSchedule.days;
}
...
export async function fetchShipheroOnholdSchedule(supabaseAdmin: SupabaseClient) {
  ...
  if (error) throw new Error(`Supabase schedule fetch failed: ${error.message}`);

  return normalizeShipheroOnholdSchedule(
    (data?.schedule as Partial<ShipheroOnholdSchedule> | null) ?? defaultShipheroOnholdSchedule
  );
}
...
export async function saveShipheroOnholdSchedule(
  ...
  const { error } = await supabaseAdmin.from("app_sync_schedules").upsert(
    { name: SCHEDULE_NAME, schedule: normalized, updated_at: new Date().toISOString() },
    { onConflict: "name" }
  );
