...
function getComputedNeededQty(row: InventoryRow, minimumStockDays: number) {
  ...
  const targetInventory =
    runningAverage * (Math.max(0, minimumStockDays) + leadTimeDays);
...
export default function InventoryPage() {
  ...
  const categories = useMemo(
    () => Array.from(new Set(inventoryRows.map((item) => item.category))),
    []
  );

  const vendors = useMemo(
    () => Array.from(new Set(inventoryRows.map((item) => item.vendor))),
    []
  );
  ...
  const rowsWithApprovedQty = useMemo(() => {
    return filteredRows.filter(
      (item) => Number(approvedQtyBySku[item.sku] ?? 0) > 0
    );
  ...
  const setApprovedQty = (sku: string, value: number) => {
    setApprovedQtyBySku((prev) => ({
      ...prev,
      [sku]: Math.max(0, Number(value) || 0),
    }));
  };
  ...
  const bumpApprovedQty = (item: InventoryRow, direction: 1 | -1) => {
    ...
    const next =
      direction === 1 ? current + multiple : Math.max(0, current - multiple);
    ...
    setNewRows(
      rows
        ...
        .map((row) => ({
          ...
          amount: Number(
            ((approvedQtyBySku[row.sku] ?? 0) * row.cost).toFixed(2)
          ),
        }))
    );
    ...
    setNewRows((prev) =>
      prev.map((row) => {
        ...
        if (field === "itemDescription") {
          const selectedItem = inventoryRows.find(
        ...
        if (field === "ordered") {
          ...
          return {
            ...
            ordered,
            amount: sourceItem
              ? Number((ordered * sourceItem.cost).toFixed(2))
              : row.amount,
          };
        }
    ...
    <div className="space-y-6">
      ...
      {vendor !== "All" && rowsWithApprovedQty.length > 0 && (
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-3 text-sm text-emerald-900">
          Showing {rowsWithApprovedQty.length} approved item
          {rowsWithApprovedQty.length === 1 ? "" : "s"} for {vendor}. Click
          Create PO to create a purchase order from the approved quantities.
        </div>
      )}
              ...
              {filteredRows.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap px-4 py-4 text-right tabular-nums">
                      {getRunningAverage90Days(item)}
                    </td>
                    <td className="whitespace-nowrap px-4 py-4 text-right tabular-nums">
                      {getLeadTimeDays(item)} days
                    </td>
                    <td className="whitespace-nowrap px-4 py-4 text-right tabular-nums">
                      {getStockLevelDays(item)}
                    </td>
                        ...
                        <td className="px-4 py-3">
                          ...
                          <datalist id="item-options">
                            {inventoryRows.map((item) => (
                              <option key={item.sku} value={item.itemName} />
                            ))}
                          </datalist>
                        </td>
