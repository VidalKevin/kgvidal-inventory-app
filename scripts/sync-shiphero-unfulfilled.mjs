...
function countBusinessDays(orderDateIso, todayDate) {
  let count = 0;
...
async function extractUnfulfilledOrders(page) {
  return page.evaluate(() => {
    ...
    const rows = Array.from(table.querySelectorAll("tbody tr")).map((tr) => {
      ...
      const orderLink = Array.from(tr.querySelectorAll("a"))
        .map((anchor) => ({
          text: anchor.textContent.trim().replace(/\s+/g, " "),
          href: anchor.href,
        }))
        .find((anchor) => /#?[A-Z]*\d+/i.test(anchor.text) || /orders/i.test(anchor.href));
    ...
    const orders = rows
      ...
      .map((row) => ({
        ...
        order_number:
          orderIndex >= 0
            ? row.cells[orderIndex] || row.orderLink?.text || null
            : row.orderLink?.text || null,
        order_url: row.orderLink?.href || null,
      }))
...
async function findBackorderedProductNames(page) {
  return page.evaluate(() => {
    const normalize = (value) => value.trim().replace(/\s+/g, " ").toLowerCase();
    ...
    for (const table of Array.from(document.querySelectorAll("table"))) {
      ...
      const backorderedIndex = headers.findIndex(
        (header) =>
          header.includes("total backordered") || header.includes("backordered")
      );

      if (backorderedIndex < 0) {
        continue;
      }
      ...
      for (const tr of Array.from(table.querySelectorAll("tbody tr"))) {
        ...
        if (parseQuantity(cells[backorderedIndex]) > 0) {
          names.push(cells[productIndex >= 0 ? productIndex : 0]);
        }
      }
...
async function getReasonForDelay(page, order) {
  const orderNumber = String(order.order_number ?? "").trim();
  const cleanOrderNumber = orderNumber.replace(/^#/, "");
  const detailUrl =
    order.order_url ||
    `https://app.shiphero.com/dashboard/orders/v2/manage/${encodeURIComponent(cleanOrderNumber)}`;

  try {
    await page.goto(detailUrl, { waitUntil: "domcontentloaded", timeout: 90000 });
    await page.waitForTimeout(3000);
    await page.waitForFunction(
      () => /backordered|sku|product|item|order/i.test(document.body?.textContent ?? ""),
      null,
      { timeout: 45000 }
    );

    const names = await findBackorderedProductNames(page);

    if (names.length === 0) {
      return "Unknown";
    }

    return `${Array.from(new Set(names)).join(", ")} Out of Stocks`;
  } catch (error) {
    console.warn(
      `Unable to inspect delay reason for ${orderNumber}: ${
...
async function enrichUnfulfilledOrders(page, orders) {
  const todayDate = new Date();
  todayDate.setHours(0, 0, 0, 0);
  const enriched = [];

  for (const order of orders) {
    const orderDateIso = toIsoDate(order.order_date);
    const daysDelayed = orderDateIso ? countBusinessDays(orderDateIso, todayDate) : 0;
    const reasonForDelay =
      daysDelayed >= 2 ? await getReasonForDelay(page, order) : "N/A";
...
async function saveOrdersToSupabase(orders) {
  ...
  const { error: deleteError } = await supabaseAdmin
    .from("shiphero_unfulfilled_orders")
    .delete()
    .neq("id", 0);
  ...
  const rows = orders.map((order) => ({
    ...
    reason_for_delay: order.reason_for_delay,
    synced_at: syncedAt,
  }));
...
async function main() {
  ...
  try {
    ...
    const result = await extractUnfulfilledOrders(page);
    const enrichedOrders = await enrichUnfulfilledOrders(page, result.orders);
    const saved = await saveOrdersToSupabase(enrichedOrders);
      ...
      JSON.stringify(
        {
          ...
          ordersEnriched: enrichedOrders.length,
          saved,
        },
        null,
        2
